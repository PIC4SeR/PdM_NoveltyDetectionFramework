...
int TcThresholdsAgent::fRun() {
	...
	for (float rMeasureValue : *cMeasureValues) {
		...
		if (rNewState != TcThresholdsAgent::kMeasureInRange && rNewState != this->rmState) {
			...
			fGetNotificationByComparing(rNewState, this->rmSensorPosition, this->rmEquipmentFamily, this->rmEquipmentSN, rMeasureValue, rMeasureAlarmThresholdHigh, rMeasureAlarmThresholdLow, this->rmMeasureType, this->rmMeasureUnit, &rMeasureNotificationBody, &rMeasureNotificationType, &rMeasureNotificationTitle);
			rResult = fNotifyMessage(rMeasureNotificationType, rMeasureNotificationTitle, rMeasureNotificationBody, this->rmEquipmentSN, this->rmEquipmentFamily);
			if (rResult < 0) {
