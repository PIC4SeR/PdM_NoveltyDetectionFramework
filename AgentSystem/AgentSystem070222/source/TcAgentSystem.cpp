...
int main()
{
	...
	cThreasholdMap.insert(make_pair<string, vector<float>>("Humidi ty", { 60.0, 20.0, 80.0, 0.0 }));
	...
	/*for (int i = 0; i < 11; i++) {
		for (int j = 0; j < 3;  j++) {
			system->fAddAgentToManager(new TcThresholdsAgent(0, rPositions[i], rMeasureUnits[j], rMeasureTypes[j], rReferenceIDs[i], rReferenceType, cThreasholdMap.at(rMeasureTypes[j])[0], cThreasholdMap.at(rMeasureTypes[j])[1], cThreasholdMap.at(rMeasureTypes[j])[2], cThreasholdMap.at(rMeasureTypes[j])[3], "4080", rEquipmentSerialNumber,
	...
	/*system->fAddAgentToManager(new TcThresholdsComputeAgent(0, rPositions[7], rMeasureUnits[3], rMeasureTypes[3], rReferenceIDs[7], rReferenceType, cThreasholdMap.at(rMeasureTypes[3])[0], cThreasholdMap.at(rMeasureTypes[3])[1], cThreasholdMap.at(rMeasureTypes[3])[2], cThreasholdMap.at(rMeasureTypes[3])[3], "4080", rEquipmentSerialNumber,
		rMongoDBDatabasename, rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
		rNotifierHost, rNotifierPort,
		rAgentIDs[3], rAgentNames[3], chrono::microseconds(5000000))
	);

	system->fAddAgentToManager(new TcThresholdsComputeAgent(0, rPositions[10], rMeasureUnits[3], rMeasureTypes[3], rReferenceIDs[10], rReferenceType, cThreasholdMap.at(rMeasureTypes[3])[0], cThreasholdMap.at(rMeasureTypes[3])[1], cThreasholdMap.at(rMeasureTypes[3])[2], cThreasholdMap.at(rMeasureTypes[3])[3], "4080", rEquipmentSerialNumber, rMongoDBDatabasename,
	...
	/*for (int j = 4; j < 13; j++) {
		system->fAddAgentToManager(new TcThresholdsAgent(0, rPositions[11], rMeasureUnits[j], rMeasureTypes[j], rReferenceIDs[11], rReferenceType, cThreasholdMap.at(rMeasureTypes[j])[0], cThreasholdMap.at(rMeasureTypes[j])[1], cThreasholdMap.at(rMeasureTypes[j])[2], cThreasholdMap.at(rMeasureTypes[j])[3], "4080", rEquipmentSerialNumber,
			rMongoDBDatabasename, rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
			rNotifierHost, rNotifierPort,
			rAgentIDs[j], rAgentNames[j], chrono::microseconds(5000000))
		);
	...
	/*system->fAddAgentToManager(new TcAgent(rEquipmentSerialNumber, "4080", rMongoDBDatabasename,
		rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
		rNotifierHost, rNotifierPort,
		"AP0", "Agente prova", chrono::microseconds(5000000))
	);*/

	system->fAddAgentToManager(new TcMeasurementsDeviationDetectorAgent(rEquipmentSerialNumber, "Equipment", rEquipmentSerialNumber, "4080", rMongoDBDatabasename, "Measures",
		rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
		rNotifierHost, rNotifierPort,
