...
TcThresholdsAgent::TcThresholdsAgent(int pState, string pMachineFamily, string pEquipmentSN, string pSensorPosition, string pMeasureUnit, string pMeasuretype, string pReferenceid, string pReferenceType, float pWarningThresholdHigh, float pWarningThresholdLow, float pAlarmThresholdHigh, float pAlarmThresholdLow, string pCollectionname, string pDatabasename, string pMongoDriverRemoteConnectionType, string pMongoDriverRemoteConnectionHost, uint16_t pMongoDriverRemoteConnectionPort, string pNotifyDriverRemoteConnectionHost, uint16_t pNotifyDriverRemoteConnectionPort, string agentid, string agentname, chrono::microseconds stepruntime, Priority priority, bool stopped) : TcAgent(pCollectionname, pDatabasename, pMongoDriverRemoteConnectionType, pMongoDriverRemoteConnectionHost, pMongoDriverRemoteConnectionPort, pNotifyDriverRemoteConnectionHost, pNotifyDriverRemoteConnectionPort, agentid, agentname, stepruntime, priority, stopped){
	this->rmState = pState;
...
int TcThresholdsAgent::fRun() {
	...
	for (float rMeasureValue : *cMeasureValues) {
		...
		if (rNewState != TcThresholdsAgent::kMeasureInRange && rNewState != this->rmState) {
			...
			fGetNotificationByComparing(rNewState, this->rmSensorPosition, this->rmMachineFamily, this->rmEquipmentSN, rMeasureValue, rMeasureAlarmThresholdHigh, rMeasureAlarmThresholdLow, this->rmMeasureType, this->rmMeasureUnit, &rMeasureNotificationBody, &rMeasureNotificationType, &rMeasureNotificationTitle);
...
int TcThresholdsAgent::fGetData(list<float>** pMeasureValues) {
	...
	if (this->cmMongoInterface->fGetMeasuresBetweenValues(&cMeasureBetweenTimeValues, this->rmDatabasename, this->rmCollectionname, "TimestampEpoch", to_string(rToMilliseconds), to_string(rFromMilliseconds), this->rmReferenceID, this->rmReferenceType, this->rmMeasureType, "TimestampEpoch", "Value") < 0) {
...
void TcThresholdsAgent::fGetNotificationByComparing(int pComparingResult, string pSensorPosition, string pMachineFamily, string pEquipmentSN, float pValue, float pAlarmThresholdHigh, float pAlarmThresholdLow, string pMeasureType, string pMeasureUnit, string *pNotificationBody, string* pNotificationType, string* pNotificationTitle) {
	...
	char aStringRoundedWarningThresholdLow[16];
	char aStringRoundedWarningThresholdHigh[16];
	...
	string rDefaultTitle = "on " + pMachineFamily + " SN: " + pEquipmentSN;
