...
void TcAgentSystem::fAddAgentToManager(IAgent* pAgent) {
...
int main()
{
	...
	system->fLoadManager("AM0", "Agent Manager", chrono::microseconds(1000000), chrono::microseconds(10000000));

	/*for (int i = 0; i < 11; i++) {
		for (int j = 0; j < 3;  j++) {
			system->fAddAgentToManager(new TcThresholdsAgent(0, "4080", rEquipmentSerialNumber, rPositions[i], rMeasureUnits[j], rMeasureTypes[j], rReferenceIDs[i], rReferenceType, cThreasholdMap.at(rMeasureTypes[j])[0], cThreasholdMap.at(rMeasureTypes[j])[1], cThreasholdMap.at(rMeasureTypes[j])[2], cThreasholdMap.at(rMeasureTypes[j])[3], rMongoDBCollectionname, rMongoDBDatabasename,
				rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
				rNotifierHost, rNotifierPort,
				rAgentIDs[j], rAgentNames[j], chrono::microseconds(5000000), Priority::VeryHigh, false)
			);
		}
	}

	system->fAddAgentToManager(new TcThresholdsComputeAgent(0, "4080", rEquipmentSerialNumber, rPositions[7], rMeasureUnits[3], rMeasureTypes[3], rReferenceIDs[7], rReferenceType, cThreasholdMap.at(rMeasureTypes[3])[0], cThreasholdMap.at(rMeasureTypes[3])[1], cThreasholdMap.at(rMeasureTypes[3])[2], cThreasholdMap.at(rMeasureTypes[3])[3], rMongoDBCollectionname, rMongoDBDatabasename,
		rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
		rNotifierHost, rNotifierPort,
		rAgentIDs[3], rAgentNames[3], chrono::microseconds(5000000), Priority::VeryHigh, false)
	);

	system->fAddAgentToManager(new TcThresholdsComputeAgent(0, "4080", rEquipmentSerialNumber, rPositions[10], rMeasureUnits[3], rMeasureTypes[3], rReferenceIDs[10], rReferenceType, cThreasholdMap.at(rMeasureTypes[3])[0], cThreasholdMap.at(rMeasureTypes[3])[1], cThreasholdMap.at(rMeasureTypes[3])[2], cThreasholdMap.at(rMeasureTypes[3])[3], rMongoDBCollectionname, rMongoDBDatabasename,
		rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
		rNotifierHost, rNotifierPort,
		rAgentIDs[3], rAgentNames[3], chrono::microseconds(5000000), Priority::VeryHigh, false)
	);
	...
	for (int j = 4; j < 13; j++) {
		system->fAddAgentToManager(new TcThresholdsAgent(0, "4080", rEquipmentSerialNumber, rPositions[11], rMeasureUnits[j], rMeasureTypes[j], rReferenceIDs[11], rReferenceType, cThreasholdMap.at(rMeasureTypes[j])[0], cThreasholdMap.at(rMeasureTypes[j])[1], cThreasholdMap.at(rMeasureTypes[j])[2], cThreasholdMap.at(rMeasureTypes[j])[3], rMongoDBCollectionname, rMongoDBDatabasename,
			rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
			rNotifierHost, rNotifierPort,
			rAgentIDs[j], rAgentNames[j], chrono::microseconds(5000000), Priority::VeryHigh, false)
		);
	...
	/*system->fAddAgentToManager(new TcAgent(rMongoDBCollectionname, rMongoDBDatabasename,
		rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
		rNotifierHost, rNotifierPort,
		"AP0", "Agente prova", chrono::microseconds(5000000), Priority::VeryHigh, false)
	);*/

	system->fAddAgentToManager(new TcMeasurementsDeviationDetectorAgent("4080", "agrdt803", "agrdt803", "", rMongoDBCollectionname, rMongoDBDatabasename,
		rMongoDBConnectionType, rMongoDBConnectionHost, rMongoDBConnectionPort,
		rNotifierHost, rNotifierPort,
		"AP0", "Agente prova", chrono::microseconds(30000000), Priority::VeryHigh, false)
	);


	thread* managerThread = new thread(&TcAgentSystem::fExecuteManager, system);
	managerThread->join();
