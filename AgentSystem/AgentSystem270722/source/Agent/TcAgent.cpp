...
int TcAgent::fGetData(list<string>* pLastTemperatures, string* pAvgTemperature, string* pMaxTemperature, string* pMinTemperature, string* pSumTemperatures, list<string>* pTemperaturesBetween, list<string>* pTemperaturesGreatherThan, list<string>* pTemperaturesLowerThan, string* pAvgTemperatureBetweenValues, string* pAvgTemperatureGreatherThanValue, string* pAvgTemperatureLowerThanValue, string* pMaxTemperatureBetweenValues, string* pMaxTemperatureGreatherThanValue, string* pMaxTemperatureLowerThanValue, string* pMinTemperatureBetweenValues, string* pMinTemperatureGreatherThanValue, string* pMinTemperatureLowerThanValue, string* pSumTemperatureBetweenValues, string* pSumTemperatureGreatherThanValue, string* pSumTemperatureLowerThanValue, string* pCountTemperatureBetweenValues, string* pCountTemperatureGreatherThanValue, string* pCountTemperatureLowerThanValue) {
	...
	int rResult = this->cmMongoInterface->fConnectDriver();
	if (rResult < 0) {
	...
	auto t = v.view()["rpTimestamp"].type();
	auto d = v.view()["rpTimestamp"].get_date();
	*/
	...
	if (this->cmMongoInterface->fGetMeasuresInBackWindow<TcMeasure>(&cTemperaturesBetweenTimePoints, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", 100, "", "Sensor", "Temp", "rpTimestamp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}
	*/
	...
	if (this->cmMongoInterface->fGetMeasuresBetweenTimePoints(&cTemperaturesBetweenTimePoints, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, from, "Sensor", "Temp", "rpTimestamp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	list<string> cTemperaturesAfterTimePoint;
	if (this->cmMongoInterface->fGetMeasuresAfterTimePoint(&cTemperaturesAfterTimePoint, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", from, "Sensor", "Temp", "rpTimestamp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	list<string> cTemperaturesBeforeTimePoint;
	if (this->cmMongoInterface->fGetMeasuresBeforeTimePoint(&cTemperaturesBeforeTimePoint, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, "Sensor", "Temp", "rpTimestamp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cMinTemperatureBetweenTimePoints;
	if (this->cmMongoInterface->fGetMinMeasureBetweenTimePoints(&cMinTemperatureBetweenTimePoints, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, from, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cMinTemperatureAfterTimePoints;
	if (this->cmMongoInterface->fGetMinMeasureAfterTimePoint(&cMinTemperatureBetweenTimePoints, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", from, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cMinTemperatureBeforeTimePoint;
	if (this->cmMongoInterface->fGetMinMeasureBeforeTimePoint(&cMinTemperatureBetweenTimePoints, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cMaxTemperatureBetweenTimePoints;
	if (this->cmMongoInterface->fGetMaxMeasureBetweenTimePoints(&cMaxTemperatureBetweenTimePoints, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, from, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cMaxTemperatureAfterTimePoint;
	if (this->cmMongoInterface->fGetMaxMeasureAfterTimePoint(&cMaxTemperatureAfterTimePoint, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", from, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}
	...
	if (this->cmMongoInterface->fGetMaxMeasureBeforeTimePoint(&cMaxTemperatureBeforeTimePoint, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cAvgTemperatureBetweenTimePoints;
	if (this->cmMongoInterface->fGetAvgMeasureBetweenTimePoints(&cAvgTemperatureBetweenTimePoints, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, from, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cAvgTemperatureAfterTimePoint;
	if (this->cmMongoInterface->fGetAvgMeasureAfterTimePoint(&cAvgTemperatureAfterTimePoint, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", from, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cAvgTemperatureBeforeTimePoint;
	if (this->cmMongoInterface->fGetAvgMeasureBeforeTimePoint(&cAvgTemperatureBeforeTimePoint, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", to, "Value", "Sensor", "Temp") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}*/
	...
	if (this->cmMongoInterface->fGetMeasuresInBackWindow(&cTemperaturesInBackWindow, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", 100, "Sensor", "Temp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cAvgTemperatureInBackWindow;
	if (this->cmMongoInterface->fGetAvgOfMeasuresInBackWindow(&cAvgTemperatureInBackWindow, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", 100, "Value", "Sensor", "Temp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cMaxTemperatureInBackWindow;
	if (this->cmMongoInterface->fGetMaxOfMeasuresInBackWindow(&cMaxTemperatureInBackWindow, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", 100, "Value", "Sensor", "Temp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	string cMinTemperatureInBackWindow;
	if (this->cmMongoInterface->fGetMinOfMeasuresInBackWindow(&cMinTemperatureInBackWindow, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", 100, "Value", "Sensor", "Temp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}
	
	string cSumTemperatureInBackWindow;
	if (this->cmMongoInterface->fGetSumOfMeasuresInBackWindow(&cSumTemperatureInBackWindow, "SpeaTestDB", TcMeasure::kCollection, "rpTimestamp", 100, "Value", "Sensor", "Temp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	list<string> cTemperaturesBetweenValues;
	if (this->cmMongoInterface->fGetMeasuresBetweenValues(&cTemperaturesBetweenValues, "SpeaTestDB", TcMeasure::kCollection, "Value", "33", "28", "Sensor", "Temp", "rpTimestamp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	list<string> cTemperaturesGreatherThanValue;
	if (this->cmMongoInterface->fGetMeasuresGreatherThanValue(&cTemperaturesGreatherThanValue, "SpeaTestDB", TcMeasure::kCollection, "Value", "28", "Sensor", "Temp", "rpTimestamp", "Value") < 0) {
		this->cmMongoInterface->fDisconnectDriver();
		return(kGetDataFails);
	}

	list<string> cTemperaturesLowerThanValue;
	if (this->cmMongoInterface->fGetMeasuresLowerThanValue(&cTemperaturesLowerThanValue, "SpeaTestDB", TcMeasure::kCollection, "Value", "33", "Sensor", "Temp", "rpTimestamp", "Value") < 0) {
		return(kGetDataFails);
	}
