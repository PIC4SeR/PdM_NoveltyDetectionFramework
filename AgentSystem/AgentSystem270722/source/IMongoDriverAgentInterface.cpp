...
void IMongoDriverAgentInterface::fAddFictitiousData() {
    for (int i = 0; i < 100; i++) {
        bsoncxx::document::value Doc = bsoncxx::builder::stream::document{} << "ReferenceID" << "000001" << "ReferenceType" << "Sensor" << "rpTimestamp" << bsoncxx::types::b_date{ chrono::system_clock::now() } << "MeasureUnit" << "T" << "MeasureType" << "Temp" << "DrawReference" << "1" << "RelayProbeType" << "1" << "StressIndex" << "1" << "Value" << 10 << "HighLimit" << 10 << "LowLimit" << 8 << "Result" << "10" << "TPGMName" << "tpgm1" << "ProductSN" << "sn0000000001" << "Task" << "t1" << "Test" << "t1" << "TestPoint" << "t1" << "rpRemark" << "descr" << bsoncxx::builder::stream::finalize;
        string SDoc = bsoncxx::to_json(Doc);
        this->cmMongoDriver->fInserDocument("SpeaTestDB", TcMeasure::kCollection, SDoc);
    }
}
...
int IMongoDriverAgentInterface::fGetStatisticsOfMeasuresGreatherThanValue(list<TcStatisticTestMeasures>* pOutput, string pDatabase, string pCollection, string pCompareattribute, string pComparevalue, int pWindow, string pMovavgattribute, string pReferenceid, string pReferencetype, string pMeasuretype, string pGroupattribute, string pProjectionattribute) {
    ...
    if (pGroupattribute == "") {
        ...
        rGroupString = "{\"_id\": { \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kTest + "\" : \"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kTest + "\"  } ,\
            \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kTest + "\" : { \"$first\": \"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kTest + "\" },\
            \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMeasures + "\"  : { \"$push\": \"$$ROOT\"},\
            \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kCalibration + "\"   : { \"$push\": {\"" + TcMeasure::TcMeasureAttributes::kValue + "\": { \"$divide\": [{ \"$sum\": [\"$" + TcMeasure::TcMeasureAttributes::kHighLimit + "\" , \"$" + TcMeasure::TcMeasureAttributes::kLowLimit + "\" ] }, 2] }}},\
            \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kErrors + "\"   : { \"$push\": { \"" + TcMeasure::TcMeasureAttributes::kValue + "\" : { \"$pow\": [{ \"$subtract\": [{ \"$divide\": [{ \"$sum\": [\"$" + TcMeasure::TcMeasureAttributes::kHighLimit + "\" , \"$" + TcMeasure::TcMeasureAttributes::kLowLimit + "\" ] }, 2] }, \"$" + TcMeasure::TcMeasureAttributes::kValue + "\" ] }, 2] }}},\
            \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMean + "\"   : { \"$avg\": \"$" + TcMeasure::TcMeasureAttributes::kValue + "\" },\
            \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kStdDev + "\"   : { \"$stdDevSamp\": \"$" + TcMeasure::TcMeasureAttributes::kValue + "\"}}";
    }
    ...
    if (pMovavgattribute == "") {
        ...
        rAddfieldsString = "{\"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kWindow + "\" :" + to_string(pWindow) + ", \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMSE + "\" : {\"$avg\": \"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kErrors + "." + TcMeasure::TcMeasureAttributes::kValue + "\"}, \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kStatistics + "\": {\
                \"$map\": {\
                    \"input\": {\"$range\": [0, { \"$subtract\": [{\"$size\":\"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMeasures + "\"}, " + to_string(pWindow - 1) + "] }] },\
                    \"as\" : \"index\",\
                    \"in\" : {\
                        \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMean + "\": { \"$avg\": {\"$slice\": [\"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMeasures + "." + TcMeasure::TcMeasureAttributes::kValue + "\", \"$$index\"," + to_string(pWindow) + "] } },\
                        \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kVariance + "\": { \"$pow\": [{ \"$stdDevSamp\": {\"$slice\": [\"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMeasures + "." + TcMeasure::TcMeasureAttributes::kValue + "\", \"$$index\"," + to_string(pWindow) + "] } }, 2]},\
                        \"" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMSE + "\": { \"$avg\": {\"$slice\": [\"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kErrors + "." + TcMeasure::TcMeasureAttributes::kValue + "\", \"$$index\"," + to_string(pWindow) + "] } },\
                        \"" + TcMeasure::TcMeasureAttributes::kTimestampEpoch + "\" : { \"$arrayElemAt\": [\"$" + TcStatisticTestMeasures::TcStatisticTestMeasuresAttributes::kMeasures + "." + TcMeasure::TcMeasureAttributes::kTimestampEpoch + "\", { \"$add\": [\"$$index\", " + to_string(pWindow - 1) + "] }] }\
                    }\
                }\
            }\
        }";
...
int IMongoDriverAgentInterface::fGetMeasuresBetweenTimePoints(list<string>* output, string database, string collection, string betweenattribute, chrono::time_point<chrono::high_resolution_clock> to, chrono::time_point<chrono::high_resolution_clock> from, string referencetype, string measuretype, string sortattribute, string projectionattribute) {
    ...
    chrono::time_point<chrono::system_clock, chrono::milliseconds> cEquality_system = chrono::system_clock::now();
