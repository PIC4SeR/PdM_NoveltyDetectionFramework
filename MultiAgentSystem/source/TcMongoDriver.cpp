...
int TcMongoDriver::fClientAcquire(boost::optional<mongocxx::pool::entry>* pOptionalMongoEntry) {
	try {
		...
		mongocxx::pool cMongoDriverConnectionPool = mongocxx::pool(cMongoDriverConnectionUri);
		
		//fprintf(stdout, "(%s) Enter in %s \n", __func__, __func__);
		//fflush(stdout);

		while (rRetryConnection < rRetryMaxConnection) {
			*pOptionalMongoEntry = cMongoDriverConnectionPool.try_acquire();
...
int TcMongoDriver::fInserDocument(string pDatabase, string pCollection, bsoncxx::document::view_or_value pBsonDocument) {
	...
	*/
	cMongoCollection = cMongoDatabase[pCollection];
	if (!cMongoCollection) {
		fprintf(stdout, ANSI_COLOR_RED "(%s) Mongo collectiopn invalid" ANSI_COLOR_RESET "\n", __func__);
		fflush(stdout);
		return(-1);
	}

	try {
		boost::optional<mongocxx::result::insert_one> cResult = cMongoCollection.insert_one(pBsonDocument.view());
	...
	*/
	...
	try {
		...
		*pOutputList = cOutputList;
		...
		fprintf(stdout, ANSI_COLOR_RED "(%s) Catched exception - Message %s" ANSI_COLOR_RESET "\n", __func__, e.what());
		fflush(stdout);
		return(kErr_Insert);
	}
	catch (mongocxx::query_exception qe) {
